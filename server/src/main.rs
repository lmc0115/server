...
fn handle_connection(mut stream: TcpStream) {
    ...
    loop {
        // Read next HTTP request on this connection
        let bytes_read = match stream.read(&mut buf) {
            Ok(0) => break,   // client closed connection
            Ok(n) => n,
            Err(_) => return, // error, drop connection
        };
        ...
        match (method, path) {
            ...
            ("GET", "/count") => {
                ...
                respond(&mut stream, "200 OK", &body, "text/plain");
            }
        ...
        // loop continues: same TcpStream can handle another request
    }
}
